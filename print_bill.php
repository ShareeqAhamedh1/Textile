...
if ($rs_ord->num_rows > 0) {
    while ($rowOrd = $rs_ord->fetch_assoc()) {
        ...
        $discount_per_item = $rowOrd['discount'] ?? 0;
        $line_discount = $discount_per_item * $quantity;
        $line_total = ($p_price * $quantity) - $line_discount;
...
<br>
  ...
  <div class="totals">
    ...
    <?php if ($total_discount + $discount_price_bill > 0) { ?>
    ...
    <?php if($cash_took > 0){ ?>
      ...
      <div><strong>Balance Paid: Rs <?= number_format($cash_took - $finalTotal) ?>/-</strong></div>
    <?php } ?>
  </div>
